...
namespace Akshay.Controllers
{
    ...
    public class TransacaoController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTransactionWithInvestor()
        {
            using (var sqlConnection = new SqlConnection(_connectionString))
            {
                const string sql = "SELECT Transactions.id, Transactions.investment_id, Transactions.transaction_date, Transactions.transaction_amount, " +
                    "Transactions.transaction_type, Transactions.trader Products.name, Products.description, Products.rentability, " +
                    "Products.category_id FROM Transactions INNER JOIN Products ON Products.ID = Transactions.product_id";
